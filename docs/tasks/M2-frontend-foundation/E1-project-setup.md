...
**Description**: Create three utility modules: `constants.ts` (API base URL, localStorage key prefix `recruitai_`, file upload limits), `storage.ts` (typed localStorage helpers for reading/writing JSON with the `recruitai_` prefix), `api.ts` (auth verification fetch wrapper).
**Acceptance criteria**:
