...
**Description**: Create a generic typed React hook for reading and writing values to localStorage. All keys are automatically prefixed with `hiro_`. The hook returns `[value, setValue]` similar to `useState`, and syncs with localStorage on every update. Initialize from localStorage on mount, falling back to the provided default value.
**Acceptance criteria**:

-   [ ] `useLocalStorage<T>(key, defaultValue)` returns `[T, (value: T) => void]`
-   [ ] Keys are prefixed with `hiro_` automatically
...
-   [ ] Messages persist across page refreshes via localStorage (`hiro_messages`)
-   [ ] Evaluations persist across page refreshes via localStorage (`hiro_evaluations`)
...
**Description**: Add a "New Search" button (in the header area of the chat page) that triggers a confirmation dialog. On confirmation, clear all `hiro_*` keys from localStorage, reset `useChat` state, and restart the chat with a fresh AI greeting.
**Acceptance criteria**:

-   [ ] "New Search" button is visible in the chat UI
-   [ ] Clicking it shows a confirmation dialog
-   [ ] Confirming clears all `hiro_*` keys from localStorage
