# RecruitAI — Technical Design

This document maps the product requirements from [IDEA.md](./IDEA.md) to concrete architecture decisions. A developer should be able to read this and start building.
...
5. Client extracts the evaluation, saves it to localStorage under `recruitai_evaluations`
...
User types job description in ChatInput
  ...
  → Messages persisted to localStorage (recruitai_messages)
  → Job description saved to localStorage (recruitai_job_description)
```
...
User drops PDF into FileUploadArea
  ...
  → Saves to localStorage (recruitai_evaluations)
...
User clicks "New Search"
  ...
  → User confirms
  → Clear all recruitai_* keys from localStorage
  → Reset useChat state
...
|-----|------|-------------|
| `recruitai_messages` | `ChatMessage[]` | Full conversation history |
| `recruitai_job_description` | `string` | Current job description text |
| `recruitai_evaluations` | `CandidateEvaluation[]` | All candidate evaluations |
| `recruitai_uploaded_files` | `FileRecord[]` | Metadata of uploaded PDFs (name, size, timestamp) |
